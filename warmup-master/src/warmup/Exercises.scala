...
Indication of length : 12 lines
  ...
  def diamondString(height : Int) : String = {
    var numberOfSpaces : Int = (height-1)/2
    var numberOfSymbols : Int = 1

    var diamond : String = """""".stripMargin
...
Indication of length: 25 added lines. Add extra functions for conceptually definitions to organize your code.
  ...
  def hasSpeeded(speed: Double, startTime : Time, endTime: Time): Boolean ={
    if (endTime.daysSinceEpoch == startTime.daysSinceEpoch){
      if (startTime.hours >= 6 && endTime.hours < 19){
        if (speed > speedLimitDay){
          return true
        }
        else return false
      }
    }
    if (speed > speedLimitNight){
      return true
    }
    else return false
  }

  def calcSpeed(startTime: Time, endTime: Time): Double ={
    var speed : Double = 0
    val endInSeconds : Double = (endTime.seconds + endTime.minutes * 60 + endTime.hours * 3600)
    val startInSeconds : Double = (startTime.seconds+ startTime.minutes * 60 + startTime.hours * 3600)

    if (endTime.daysSinceEpoch == startTime.daysSinceEpoch){
      val timeInterval : Double =  endInSeconds - startInSeconds
      speed = distanceAB / timeInterval
    }
    else if (endTime.daysSinceEpoch == startTime.daysSinceEpoch + 1){
      val timeInterval : Double = endInSeconds + ((24 * 3600) - startInSeconds)
      speed = distanceAB / timeInterval
    }
    speed
  }
  ...
  def speedOffenders(observations: Seq[Observation]) : ArrayBuffer[SpeedOffender] = {
    ...
    val endTimeOfCar : mutable.Map[String,Time] = new mutable.HashMap()
    val result :  ArrayBuffer[SpeedOffender] = new ArrayBuffer()

    for(ob <- observations){
      if (ob.cameraSet == "A"){
        startTimeOfCar += (ob.licensePlate -> ob.time)
      }
      else endTimeOfCar += (ob.licensePlate -> ob.time)
    }

    for (end <- endTimeOfCar){
      for (start <- startTimeOfCar){
        if (start._1 == end._1){
          val licensePlate = start._1
          val startTime = start._2
          val endTime = end._2
          val speed = calcSpeed(startTime, endTime)

          if(hasSpeeded(Math.round(speed).toInt, startTime, endTime)){
            result += SpeedOffender(licensePlate, Math.round(speed*3.6).toInt)
          }
        }
      }
    }
...
Indication of length: 12 lines
  ...
  def splitArray(a : Array[Int]) : (Array[Int],Array[Int]) = {
    ...
    for(el <- 0 until a.length){
      if(el < a.length/2){
        firstArray = firstArray :+ a(el)
      }
      else secondArray = secondArray :+ a(el)
    }
  ...
  def mergeSortedArrays(a : Array[Int], b : Array[Int]) : Array[Int] = {
    ...
    while(result.length < (a.length + b.length) ){
      if( a(pointerA) <= b(pointerB) ){
        result = result :+ a(pointerA)
        if(pointerA < a.length - 1){
          pointerA += 1
        }
        else {
          for (p <- pointerB until b.length){
            result = result :+ b(p)
          }
        }
      }
      else {
        result = result :+ b(pointerB)
        if (pointerB < b.length - 1) {
          pointerB += 1
        }
        else {
          for (p <- pointerA until a.length) {
            result = result :+ a(p)
          }
        }
      }
...
Indication of length: 5 lines
  ...
  def mergeSort(a : Array[Int]) : Array[Int] = {
    if (sorted(a)){
      ...
      val SplitArray : (Array[Int], Array[Int]) = splitArray(a)
      mergeSortedArrays(mergeSort(SplitArray._1), mergeSort(SplitArray._2))
    }
  }
